"""
Highrise Room Management Bot - @linxnyx's room
Core features: emotes 1-254, owner tipping commands, !bed / !summon teleport,
and a periodic welcome + emote-instructions announcement.
"""
...
class Bot(BaseBot):
    ...
    def __init__(self):
        ...
        # Fixed teleport spot for !bed / !summon
        self.bed_position = Position(2.0, 12.5, 5.0, facing="FrontRight")
    ...
    async def process_tip_queue_worker(self):
        while True:
            target_id, gold_bar_tier, username, amount_label = await self.tip_queue.get()
            try:
                await self.highrise.tip_user(target_id, gold_bar_tier)
                await self.highrise.chat(f"💰 @{username} have been tipped {amount_label}!")
                # Visible gap so tips from !giveall land one by one publicly
    ...
    async def bot_position_watchdog_loop(self):
        ...
        # (e.g. after a reconnect), this teleports it back automatically.
        while True:
    ...
    async def start_announcement_loop(self) -> None:
        announcements = [
            "🎉 <color=#00FFFF><b>Welcome to the room!</b></color> Make yourself at home and have fun! 💫",
    ...
    async def handle_welcome_flow(self, user: User):
        ...
        try:
            await self.highrise.chat(f"👋 Welcome to the room @{user.username}! Type a number 1-254 to dance, '!list' for emotes, '!stop' to stop.")
        except Exception:
            pass
    ...
    async def command_handler(self, user: User, message: str, source: str):
        ...
        # DEBOUNCE LOCK: Prevent identical commands within 2.5 seconds
        now = time.time()
        ...
        is_owner = (user.username.lower() == self.owner_username.lower()) or (user.id == self.bot_id)

        # Emote triggering by number (all users)
        try:
        ...
        if clean_msg == "!bed":
        ...
        elif clean_msg.startswith("!summon "):
            try:
                ...
                for u, _ in room_users.content:
                    if u.username.lower() == target_name.lower():
                        await self.highrise.teleport(u.id, self.bed_position)
                        await self.respond(user, f"✅ @{u.username} has been summoned!", source)
                        return
                await self.respond(user, "❌ User not found in the room.", source)
            except Exception:
                await self.respond(user, "❌ Invalid format. Use: !summon @username", source)
            return
            ...
            try:
                ...
                if amount_str in TIP_MAP:
                    ...
                    for u, _ in room_users.content:
                        if u.id != self.bot_id:
                            await self.tip_queue.put((u.id, TIP_MAP[amount_str], u.username, amount_str))
                            count += 1
